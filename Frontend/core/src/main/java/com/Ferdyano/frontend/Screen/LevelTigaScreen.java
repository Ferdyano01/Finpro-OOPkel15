...
public class LevelTigaScreen implements Screen {
    ...
    private void updatePhysics(float delta) {
        ...
        // mengubah posisi kotak interaksi agar ada di bagian kepala pemain
        // ini membantu saat mencoba mengambil item yang posisinya agak tinggi
        Rectangle pBounds = player.getBounds();
        float interactionSize = 60f;
        float offsetX = (pBounds.width - interactionSize) / 2;
        float offsetY = pBounds.height - interactionSize; // digeser ke atas (kepala)
        ...
        for (int i = groundItems.size - 1; i >= 0; i--) {
            ...
            if (interactionBox.overlaps(new Rectangle(it.getX(), it.getY(), 50, 50))) {
        ...
        for (int i = 0; i < altars.size; i++) {
            if (!keySpawned[i] && interactionBox.overlaps(altars.get(i))) {
        ...
        for (int i = spawnedKeys.size - 1; i >= 0; i--) {
            if (interactionBox.overlaps(new Rectangle(spawnedKeys.get(i).getX(), spawnedKeys.get(i).getY(), 60, 60))) {
        ...
        if (isFinalPortalSpawned && interactionBox.overlaps(finalPortalBounds)) {
