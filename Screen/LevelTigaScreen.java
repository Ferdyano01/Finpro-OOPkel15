...
public class LevelTigaScreen implements Screen {
    ...
    public LevelTigaScreen(TheLostKeyGame game) {
        ...
        this.uiStage = new Stage(new FitViewport(1280, 720), game.getBatch());
    ...
    public void render(float delta) {
        ...
        // Cek Input Enter jika pesan sedang muncul
        if (isWinMessageActive && Gdx.input.isKeyJustPressed(Keys.ENTER)) {
            isWinMessageActive = false;
            isFinalPortalSpawned = true;
            if (winMessageLabel != null) winMessageLabel.remove();
        }

        if (!isWinMessageActive) {
            updatePhysics(delta);
        }
        ...
        game.getBatch().draw(bgTexture, 0, 0, 1280, 720);
        ...
        game.getBatch().draw(player.getCurrentFrame(), player.getPositionX(), player.getPositionY(), 140, 140);
    ...
    private void updatePhysics(float delta) {
        float speed = 320f;
        float vx = 0, vy = 0;
        if (Gdx.input.isKeyPressed(Keys.W)) vy = speed;
        if (Gdx.input.isKeyPressed(Keys.S)) vy = -speed;
        if (Gdx.input.isKeyPressed(Keys.A)) vx = -speed;
        if (Gdx.input.isKeyPressed(Keys.D)) vx = speed;
        player.setVelocity(vx, vy);
        player.update(delta);

        // Batu Boss (Hanya aktif jika belum menang)
        if (!isFinalPortalSpawned) {
            rockTimer += delta;
            if (rockTimer >= 1.5f) {
    ...
    private void showWinMessage() {
        ...
        winMessageLabel.setPosition(640 - winMessageLabel.getWidth()/2, 400);
        uiStage.addActor(winMessageLabel);
    }

    // --- BAGIAN YANG DIUBAH (HANDLE END) ---
    private void handleEnd(String msg) {
        if (isGameOver) return;
        isGameOver = true;

        // Ubah warna teks: Gold jika Menang, Merah jika Kalah
        Color textColor = msg.contains("SELAMAT") ? Color.GOLD : Color.RED;

        Label.LabelStyle style = new Label.LabelStyle(game.getSkin().getFont("default-font"), textColor);
        Label statusLabel = new Label(msg, style);
        statusLabel.setFontScale(1.8f);
        statusLabel.setPosition(640 - (statusLabel.getWidth() * statusLabel.getFontScaleX()) / 2, 360);
        ...
        Timer.schedule(new Timer.Task() {
            @Override public void run() {
                ...
                // Logika Pindah Screen:
                if (msg.contains("SELAMAT")) {
                    // Jika Menang, masuk ke Credits
                    game.setScreen(new CreditsScreen(game));
                } else {
                    // Jika Kalah, kembali ke Main Menu
